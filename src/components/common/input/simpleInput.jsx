...
export default function SimpleInput({
  ...
  return (
    <div className="w-full relative">
      {title && (
        <div className="text-black text-lg md:text-2xl font-semibold mb-2">{title}
         {subtitle !== "" && <span className="text-gray-500 text-xs sm:text-sm"> ({subtitle})</span>}</div>
      )}

      <input
        type={type}
        className={`w-full py-2 px-2 font-medium bg-[#F6F6F6] text-black placeholder-[#81818a] text-md md:text-lg outline-none transition-colors duration-200 ${borderColor}`}
